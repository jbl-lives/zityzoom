...
const PlaceIcons: React.FC<PlaceIconsProps> = ({ onSelectCategory, selectedCategory }) => {
  return (
    <div
      className="card2 h-auto w-full overflow-x-auto scrollbar-thin scrollbar-thumb-rose-400 scrollbar-track-gray-100 
      flex flex-row items-center justify-center gap-4 lg:flex-col lg:overflow-x-visible lg:items-center lg:justify-center lg:rounded-2xl p-3 
      rounded-xl min-w-0 bg-gray-900 xl:h-full md:h-[10vh] md:overflow-hidden"
      ...
      {category.map((item) => {
        ...
        return (
          <div
            ...
            className="group flex items-center cursor-pointer transition-all xl:group-hover:scale-105"
          >
            <div className={`xl:w-9 xl:h-9 p-2 flex items-center justify-center rounded-full transition-all duration-200
              ...
              xl:group-hover:ring-1 xl:group-hover:ring-rose-500
            `}>
              <svg
            ...
            <span className="hidden md:block absolute left-full top-1/2 -translate-y-1/2 ml-2 px-2 py-1 rounded
              bg-black text-white text-xs opacity-0 xl:group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 border-2 border-white">
              {item.name}
            </span>
          </div>
        );
